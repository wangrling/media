...
/**
 * MediaProjection作为输入，MediaCodec作为输出，Muxer写入到文件中。
 */
...
public class ScreenRecordActivity extends PlayMovieSurfaceActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button toggleRecording = findViewById(R.id.screenRecordButton);

        toggleRecording.setOnClickListener(new View.OnClickListener() {
            @RequiresApi(api = Build.VERSION_CODES.M)
            @Override
            public void onClick(View v) {
                if (v.getId() == R.id.screenRecordButton) {
                    if (muxerStarted) {
                        stopRecording();
                        ((Button) findViewById(R.id.screenRecordButton)).setText(R.string.toggleRecordingOn);
                    } else {
                        Intent permissionIntent = mediaProjectionManager.createScreenCaptureIntent();
                        startActivityForResult(permissionIntent, REQUEST_CODE_CAPTURE_PERM);
                        findViewById(R.id.screenRecordButton).setEnabled(false);
                    }
                }
            }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        if (REQUEST_CODE_CAPTURE_PERM == requestCode) {
            Button b = findViewById(R.id.screenRecordButton);
            b.setEnabled(true);
