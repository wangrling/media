...
Renderer *createRenderer() {
    GLES3Renderer *renderer = new GLES3Renderer;
    if (!renderer->init()) {
        delete renderer;
        return NULL;
    }
...
GLES3Renderer::GLES3Renderer() : mEglContext(eglGetCurrentContext()),
                                 mProgram(0), mVBState(0) {
...
float *GLES3Renderer::mapOffsetBuf() {
    glBindBuffer(GL_ARRAY_BUFFER, mVB[VB_OFFSET]);
...
void GLES3Renderer::unmapOffsetBuf() {
    glUnmapBuffer(GL_ARRAY_BUFFER);
}

float *GLES3Renderer::mapTransformBuf() {
    glBindBuffer(GL_ARRAY_BUFFER, mVB[VB_SCALEROT]);
    return (float*)glMapBufferRange(GL_ARRAY_BUFFER,
...
void GLES3Renderer::unmapTransformBuf() {
    glUnmapBuffer(GL_ARRAY_BUFFER);
}

void GLES3Renderer::draw(unsigned int numInstances) {
